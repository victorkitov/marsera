    ...
    def pruning_pass(
        ...
        for M in range(term_count, 1, -1): # M, M-1, ..., 2
            ...
            for m in range(1, M):
                ...
                coeffs = self.analytically_pseudo_solves_slae(self.symmetric_positive_matrix_correct(B.T @ B), B.T @ y)
                ...
                if lof <= self.best_lof:
                    ...
                    self.best_lof = lof
                    self.term_list = self.TermListClass(K, coeffs=coeffs)
                    # self.coeffs = coeffs
        return self
