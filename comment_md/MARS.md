...
**MARS** - это многомерная непараметрическая (чем больше данных, тем лучше работает) регрессия, которая есть обобщение **Recursive partitioning regression (RPR)**.
***
...
На следующем шаге в качестве мн-ва, из которго будем удалять б.ф., берём лучшее локальное мн-во $K^*$ с предыдущего шага и повторяем процедуру, пока не останется ==одна== константная б.ф. $B_1(x) = 1$, которая должна оставаться в любом мн-ве. В итоге получается посл-ть моделей с уменьшающимся кол-вом б.ф.
...
* используя разложение Холецкого ($L^TL$), который численно менее стабильный.
...
Матрица $V$ - симметричная и положительно определённая матрица.

*Замечание:* При формировании $V$ и $c$ производится нормализация. Т.о. $V$ в некотором смысле аналогична ковариационной матрице для $B$, однако, в $V$ нормализуется только один множитель. Возможно, это нужно для упрощения формул пересчёта, чтобы там не возникали страшные слагаемые.

Итоговая сложность **MARS** "в-тупую":
![[Pasted image 20221106171403.png]]

Для уменьшения вычислений вместо ф-ции $g$ используется ф-ция $g^{'}$, оптимизация которой приводит к тем же $lof^*$ и $t^*$, но значения $\{a\}_i$ будут другими. Преимуществом такой ф-ции заключается в том, что при изменении $t$ изменяется только одна б.ф.
